...
function migrate(database: Database.Database) {
  ...
  if (!existingEventTable) {
    database.exec(`
      ...
      event_type TEXT NOT NULL CHECK (event_type IN ('page_view', 'chat_prompt', 'resume_download')),
    ...
    `);
  } else if (!existingEventTable.sql.includes("resume_download")) {
    // SQLite cannot alter a CHECK constraint. Rebuild transactionally to retain every event.
